...
// inContainer reports whether hope is running inside a container — Docker, Podman, or
// any runtime. hope's own images bake HOPE_MANAGED=1, so its presence is the signal
// (runtime-agnostic, unlike sniffing /.dockerenv). The rootfs-vs-volume distinction —
// and its "will be lost on a recreate" warning — only makes sense in a container; run
// the binary natively and the db is just a normal file: never flagged.
func inContainer() bool { return os.Getenv("HOPE_MANAGED") != "" }

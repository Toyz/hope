...
import { LoomElement, component, styles, css, reactive, watch, unmount, app } from "@toyz/loom";
...
export class NetworkDetailModal extends LoomElement {
  ...
  private gotoContainer = (id: string) => {
    if (this.host) this.hostCtx.activeHost = this.host;
    this.close();
          ...
          {this.loading ? (
            ...
            <>
              ...
              <div class="dacts">
                ...
                <span class="grow"></span>
                {n.used_by.length ? null : <button class="pbtn danger" onClick={this.removeNet}>remove</button>}
              </div>
            </>
          )}
        </div>
